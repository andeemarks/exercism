...
class BowlingGame:
    ...
    def roll(self, pins):
        ...
        if len(temp_frames) > 10:
            print(temp_frames[9].is_strike())
            print(temp_frames[10])
            print(len(temp_frames))
            # print(pins)
            if temp_frames[9].is_normal():
                raise Exception("cannot_roll_if_game_already_has_ten_frames")
            if temp_frames[9].is_spare() and temp_frames[10].is_full():
                raise Exception("cannot_roll_after_bonus_roll_for_spare")
            if temp_frames[9].is_strike() and not temp_frames[10].is_strike() and len(temp_frames) > 11:
