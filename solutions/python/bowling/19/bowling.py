...
class BowlingGame:
    ...
    def roll(self, pins):
        ...
        self._check_if_rollable(pins)


    def score(self):
        frames = self._create_frames(self.pins)
        self._validate_frame_length(frames)
        frames = self._apply_spare_bonus(frames)
        frames = self._apply_strike_bonus(frames)

        scores = map(lambda frame: frame.total(), frames)

        return sum(scores)

    def _apply_spare_bonus(self, frames: list[Frame]) -> list[Frame]:
        for i in range(0, len(frames)):
            if frames[i].is_spare() and i < LAST_FRAME:
                frames[i] = frames[i].add_bonus(frames[i + 1].first_pin)

        return frames

    def _apply_strike_bonus(self, frames: list[Frame]) -> list[Frame]:
        for i in range(0, len(frames)):
            if frames[i].is_strike() and i < LAST_FRAME:
    ...
    def _create_frames(self, pins) -> list[Frame]:
        frames = []
        pin_i = 0
        while pin_i < len(pins):
            pin_i += self._add_pin_to_frame(pins[pin_i], pin_i, frames)

        return frames

    def _add_pin_to_frame(self, pin: int, pin_i: int, frames: list[Frame]) -> int:
        next_pin_i = 0
        if pin == STRIKE:
            ...
            try:
                frames.append(Frame(pin, pin_i, self.pins[pin_i + 1]))
                next_pin_i = 2
            except IndexError:
                # Missing second roll in frame
                frames.append(Frame(pin, pin_i))
                next_pin_i = 1
            
        return next_pin_i

    def _check_if_rollable(self, next_roll):
        if next_roll not in range(0, 11):
            raise Exception("rolls_cannot_score_negative_points & a_roll_cannot_score_more_than_10_points")

        temp_frames = self._create_frames(self.pins)
        if len(temp_frames) > 10:
            last_frame = temp_frames[LAST_FRAME]
            current_frame = temp_frames[LAST_FRAME + 1]
