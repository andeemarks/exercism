...
class Rational:
    def __init__(self, numer: int, denom: int):
        ...
        self.a = self._reduce(numer, gcd, standard_form)
        self.b = self._reduce(denom, gcd, standard_form) if numer != 0 else 1
    
    def _reduce(self, n: int, gcd: int, standard_form: int) -> int:
        return (n / gcd) * standard_form
