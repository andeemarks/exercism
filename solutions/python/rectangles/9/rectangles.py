...
def _is_rectangle(grid: list[str], top_left: list[int], bottom_right: list[int]) -> bool:
    ...
    for c in range(c1, c2 + 1):
        if c >= len(grid[r1]) or grid[r1][c] not in HORZ_SIZE_CHARS:
            return False
        if c >= len(grid[r2]) or grid[r2][c] not in HORZ_SIZE_CHARS:
            return False
    
    # Check left and right edges (should be '+' or '|')
    for r in range(r1, r2 + 1):
        if c1 >= len(grid[r]) or grid[r][c1] not in VERT_SIZE_CHARS:
            return False
        if c2 >= len(grid[r]) or grid[r][c2] not in VERT_SIZE_CHARS:
            return False
