...
def delivery_date(start, description):
    ...
    if re.match(r"[0-9]+M", description):
        delivery_dt = _handle_n_month_description(start_dt, description)

    if re.match(r"Q[1234]", description):
        delivery_dt = _handle_n_quarter_description(start_dt, description)

    return _format(delivery_dt)
    
def _handle_n_quarter_description(start_dt: datetime, description: str) -> datetime:
    ...
    last_workday = _last_workday_of_quarter(year, delivery_quarter)
    delivery_dt = datetime(year, delivery_quarter * 3, last_workday, 8, 0, 0)
...
def _handle_n_month_description(start_dt: datetime, description: str) -> datetime:
    ...
    first_workday = _first_workday_of_month_year(year, delivery_month)
    delivery_dt = datetime(year, delivery_month, first_workday, 8, 0, 0)
...
def _handle_asap_delivery(start_dt: datetime) -> str:
    ...
    delivery_dt = datetime(start_dt.year, start_dt.month, day, hour, 0, 0)
...
def _handle_end_of_week_delivery(start_dt: datetime) -> datetime:
    ...
    delivery_dt = datetime(delivery_day_dt.year, delivery_day_dt.month, delivery_day_dt.day, attributes["hour"], 0, 0)
