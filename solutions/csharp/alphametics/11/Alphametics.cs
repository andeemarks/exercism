...
public static class Alphametics
{
    ...
    private static HashSet<char> ExtractLeadingLettersFromWords(IEnumerable<string> allWords)
    {
        ...
        {
            var isLeadingLetter = word.Length > 1 && char.IsLetter(word[0]);
            if (isLeadingLetter)
    ...
    private static List<char> ExtractLettersFromWords(IEnumerable<string> allWords) =>
        [.. allWords.SelectMany(word => word).Distinct()];
    ...
    private static bool IsValidEquation( Dictionary<char, int> assignment, string[] leftWords, string rightWord)
    {
        ...
        {
            leftSum += WordToValue(assignment, word);
        }

        long rightValue = WordToValue(assignment, rightWord);
