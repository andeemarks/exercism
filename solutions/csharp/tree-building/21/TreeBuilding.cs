...
public class TreeHolder(List<Tree> nodes)
{
    ...
    public TreeHolder AssignChildren()
    {
        for (int i = 1; i < Nodes.Count; i++)
        {
            var t = Nodes.First(x => x.Id == i);
            var parent = Nodes.First(x => x.Id == t.ParentId);
    ...
    public Tree Root() => Nodes.First(t => t.Id == 0);
}
...
public static class TreeBuilder
{
    public static Tree BuildTree(IEnumerable<TreeBuildingRecord> records)
    {
        ...
        return BuildNodes(sortedRecords).AssignChildren().Root();
    }
