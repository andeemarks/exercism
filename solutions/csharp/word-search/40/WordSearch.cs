...
public class WordSearch(string grid)
{
    ...
    private void FindWordInColumns(Dictionary<string, CoordPair?> results, string word, string[] columns)
    {
        ...
        {
            FindWordInString(results, word, word, column.columnNumber + 1, column.chars, CoordMapping.T2BCoordMapper);
            FindWordInString(results, ReverseWord(word), word, column.columnNumber + 1, column.chars, CoordMapping.B2TCoordMapper);
        }
    }
    ...
    private void FindWordInLines(Dictionary<string, CoordPair?> results, string word, string[] lines)
    {
        ...
        {
            FindWordInString(results, word, word, line.lineNumber + 1, line.chars, CoordMapping.L2RCoordMapper);
            FindWordInString(results, ReverseWord(word), word, line.lineNumber + 1, line.chars, CoordMapping.R2LCoordMapper);
        }
    }

    private void FindWordInDiagonals(Dictionary<string, CoordPair?> results, string word)
    {
        FindWordInDiagonals(results, word, word, 1, CoordMapping.T2BL2RMapper);
        FindWordInDiagonals(results, word, word, -1, CoordMapping.T2BR2LMapper);

        var reversedWord = ReverseWord(word);
        FindWordInDiagonals(results, reversedWord, word, 1, CoordMapping.B2TR2LCoordMapper);
        FindWordInDiagonals(results, reversedWord, word, -1, CoordMapping.B2TL2RCoordMapper);
    }
    ...
    class CoordMapping
    {
        public static CoordPair T2BL2RMapper(int lineNumber, int colNumber, int wordLength)
        {
            return ((lineNumber + 1, colNumber + 1), (lineNumber + wordLength, colNumber + wordLength));
        }

        public static CoordPair T2BR2LMapper(int lineNumber, int colNumber, int wordLength)
        {
            return ((colNumber + 1, lineNumber + 1), (colNumber + 1 - wordLength + 1, lineNumber + wordLength));
        }

        public static CoordPair B2TR2LCoordMapper(int lineNumber, int colNumber, int wordLength)
        {
            return ((colNumber + wordLength, lineNumber + wordLength), (colNumber + 1, lineNumber + 1));
        }
        public static CoordPair B2TL2RCoordMapper(int lineNumber, int colNumber, int wordLength)
        {
            return ((colNumber + 1 - wordLength + 1, lineNumber + wordLength), (colNumber + 1, lineNumber + 1));
        }

        public static CoordPair B2TCoordMapper(int lineNumber, int wordStart, int wordLength)
        {
            return ((lineNumber, wordStart + wordLength), (lineNumber, wordStart + 1));
        }
        public static CoordPair T2BCoordMapper(int lineNumber, int wordStart, int wordLength)
        {
            return ((lineNumber, wordStart + 1), (lineNumber, wordStart + wordLength));
        }

        public static CoordPair L2RCoordMapper(int lineNumber, int wordStart, int wordLength)
        {
            return ((wordStart + 1, lineNumber), (wordStart + wordLength, lineNumber));
        }

        public static CoordPair R2LCoordMapper(int lineNumber, int wordStart, int wordLength)
        {
