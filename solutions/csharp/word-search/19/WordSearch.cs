...
public class WordSearch(string grid)
{
    ...
    private void FindWordInDiagonalsT2B(Dictionary<string, ((int, int), (int, int))?> finds, string word, int offset, Func<int, int, int, ((int, int), (int, int))> mapper)
    {
        var lines = grid.Split();
        ...
        while (currentLetterPos < allLetters.Length)
        {
            ...
            if (wordStartPos >= 0)
            {
                ...
                if (wordFound)
                {
                    finds[word] = mapper(lineNumber, colNumber, wordLength);
                }
            }
            else
            {
