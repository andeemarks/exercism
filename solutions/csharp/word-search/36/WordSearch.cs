...
public class WordSearch(string grid)
{
    ...
    public Dictionary<string, CoordPair?> Search(string[] wordsToSearchFor)
    {
        ...
        var rowLines = BuildRowlines(lines);

        foreach (var word in wordsToSearchFor)
        {
            results[word] = null;
            FindWordInLines(results, word, lines);
            FindWordInColumns(results, word, rowLines);
    ...
    private void FindWordInColumns(Dictionary<string, CoordPair?> results, string word, string[] rowLines)
    {

        var lineNumber = 1;
        foreach (var rowLine in rowLines)
        {
            FindWordInString(results, word, word, lineNumber, rowLine, T2BCoordMapper);
            FindWordInString(results, ReverseWord(word), word, lineNumber, rowLine, B2TCoordMapper);
            lineNumber++;
        }
    }

    private static string[] BuildRowlines(string[] lines)
    {
