...
public class WordSearch(string grid)
{
    ...
    private void FindWordInLines(Dictionary<string, CoordPair?> results, string word, string[] lines)
    {
        ...
        {
            FindWordHorizontally(results, word, word, lineNumber, line, L2RCoordMapper);
            var reversedWord = ReverseWord(word);
            FindWordHorizontally(results, reversedWord, word, lineNumber, line, R2LCoordMapper);
            lineNumber++;
        }
    }
    ...
    private static void FindWordHorizontally(Dictionary<string, CoordPair?> results, string word, string label, int lineNumber, string line, Func<int, int, int, CoordPair> mapper)
    {
