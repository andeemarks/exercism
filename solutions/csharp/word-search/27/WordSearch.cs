...
public class WordSearch(string grid)
{
    ...
    private void FindWordInLines(Dictionary<string, CoordPair?> results, string word, string[] lines)
    {
        ...
        {
            FindWordL2R(results, word, word, lineNumber, line, L2RCoordMapper);
            var reversedWord = ReverseWord(word);
            FindWordR2L(results, reversedWord, word, lineNumber, line, R2LCoordMapper);
            lineNumber++;
        }
    }
    ...
    private static void FindWordR2L(Dictionary<string, CoordPair?> results, string word, string label, int lineNumber, string line, Func<int, int, int, CoordPair> mapper)
    {
    ...
    private static void FindWordL2R(Dictionary<string, CoordPair?> results, string word, string label, int lineNumber, string line, Func<int, int, int, CoordPair> mapper)
    {
        var wordStart = line.IndexOf(word);
        if (wordStart >= 0)
        {
