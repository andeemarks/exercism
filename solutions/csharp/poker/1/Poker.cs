...
public static class Poker
{
    ...
    private static readonly Dictionary<string, int> ORDER_MAP = RANKED_CARDS.Select((s, i) => new { String = s, Index = i }).ToDictionary(x => x.String, x => x.Index);

    public static IEnumerable<string> BestHands(IEnumerable<string> hands)
    {
        List<Tuple<HandScore, string>> scoredHands = ScoreHands(hands);
    ...
    public static List<Tuple<HandScore, string>> ScoreHands(IEnumerable<string> hands)
    {
        ...
        foreach (var hand in hands)
        {
            ...
            Card[] cardsByRank = [.. cards.OrderBy(c => ORDER_MAP.TryGetValue(c.rank, out var value) ? value : int.MaxValue)];
            Score multipleScore = ScoreMultiples(cardsByRank);
            Score rankScore = ScoreRanks(cardsByRank);

            scoredHands.Add(new Tuple<HandScore, string>(new HandScore(multipleScore, rankScore), hand));
        }
    ...
    private static PokerHands IdentifyHand(IEnumerable<Card> cards)
    {
        ...
        if ((threeOfAKind.Count() == 1) && (pairs.Count() == 1))
        {
    ...
    private static bool CheckForStraight(IEnumerable<Card> cards, IEnumerable<string> ranks, IEnumerable<IGrouping<string, string>> groupedRanks)
    {
        ...
        if (!isOnlyUniqueRanks)
        {
            return false;
        }

        return (highestRank - lowestRank == 4);
    }
    private static bool CheckForFiveHighStraight(IEnumerable<Card> cards, IEnumerable<string> ranks, IEnumerable<IGrouping<string, string>> groupedRanks)
    {
        ...
        return hasAce & (secondHighestRank - lowestRank == 3);
    }
    ...
    private static Score ScoreRanks(IEnumerable<Card> cards)
    {
        ...
        {
            score = score.Append(Card.Ordinal(card.rank).ToString().PadLeft(2, '0'));
        }


        return (Score)ulong.Parse(score.ToString());
    }
}
...
public class HandScore(Score baseScore, Score discriminator) : IComparable<HandScore>
{
    ...
    public int CompareTo(HandScore? other)
    {
        ...
        int baseComparison = this.baseScore.CompareTo(other.baseScore);
        ...
        return this.discriminator.CompareTo(other.discriminator);
    ...
    public override bool Equals(object? obj)
    {
        ...
        return this.baseScore == other.baseScore && this.discriminator == other.discriminator;
    }
