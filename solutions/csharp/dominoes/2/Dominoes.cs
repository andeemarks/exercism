...
file class Graph
{
    ...
    private Dictionary<int, int> BuildDegrees()
    {
        ...
        foreach (var (a, b) in dominoes)
        {
            ...
            UpdateDegrees(degrees, a, b);
        }
        return degrees;
    }

    private static void UpdateDegrees(Dictionary<int, int> degrees, int a, int b)
    {
        if (a == b)
        {
    ...
    private class GraphTraversal(Dictionary<int, HashSet<int>> adjacency)
    {
        private readonly Dictionary<int, HashSet<int>> adjacency = adjacency;
