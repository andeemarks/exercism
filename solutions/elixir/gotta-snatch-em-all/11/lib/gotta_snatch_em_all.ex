defmodule GottaSnatchEmAll do
  ...
  def boring_cards(collections) do
    Enum.reduce(collections, List.first(collections), fn collection, cards -> MapSet.intersection(cards, collection) end) |>
    MapSet.to_list() |>
    Enum.sort()
  end
  ...
  defp is_shiny?(card), do: String.starts_with?(card, "Shiny")

  @spec split_shiny_cards(collection()) :: {[card()], [card()]}
  def split_shiny_cards(collection) do
    {shiny_cards, non_shiny_cards} = MapSet.split_with(collection, &is_shiny?/1)

    {MapSet.to_list(shiny_cards) |>
    Enum.sort(), MapSet.to_list(non_shiny_cards) |>
    Enum.sort()}
  end
