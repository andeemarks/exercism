defmodule Luhn do
  ...
  defp validate_digits(digits, mapper) do
    ...
    |> Enum.reverse()
    |> Enum.with_index()
    |> Enum.map(fn {n, i} -> mapper.(n, i) end)
    |> Enum.sum()
    |> rem(10) == 0
  end

  defp cc_mapper(digit, index) when rem(index, 2) == 1, do: digit
  defp cc_mapper(digit, index) when rem(index, 2) == 0, do: digit * 2

  defp sin_mapper(digit, index) when rem(index, 2) == 0, do: digit
  defp sin_mapper(digit, index) when rem(index, 2) == 1 do
    if digit * 2 > 9 do
      digit * 2 - 9
    else
      digit * 2
