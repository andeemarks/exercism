defmodule Garden do
  ...
  def info(info_string, student_names \\ [:alice, :bob, :charlie, :david, :eve, :fred, :ginny, :harriet, :ileana, :joseph, :kincaid, :larry]) do
    ...
    student_names |> Enum.sort() |> Enum.with_index() |> Map.new(fn {name, i} -> {name, student_seeds(row1, row2, i)} end)
  end
  ...
  defp student_seeds(row1, row2, i) do
    ...
    {
      seeds[String.at(row1, i*2)],
      seeds[String.at(row1, i*2+1)],
      seeds[String.at(row2, i*2)],
      seeds[String.at(row2, i*2+1)]
    } |> Tuple.to_list() |> Enum.reject(&is_nil/1) |> List.to_tuple()
  end
end
