defmodule Ledger do
  ...
  defp format_amount(locale, entry_amount_in_cents, currency, number) do
    ...
    format_amount2(locale, entry_amount_in_cents, symbol, number) |> String.pad_leading(14, " ")
  end

  defp format_amount2(:en_US, entry_amount_in_cents, symbol, number) when entry_amount_in_cents >= 0 do
    "  #{symbol}#{number} "
  end
  defp format_amount2(_, entry_amount_in_cents, symbol, number) when entry_amount_in_cents >= 0 do
    " #{symbol} #{number} "
  end
  defp format_amount2(:en_US, _entry_amount_in_cents, symbol, number) do
    " (#{symbol}#{number})"
  end
  defp format_amount2(_, _entry_amount_in_cents, symbol, number) do
    " #{symbol} -#{number} "
  end
