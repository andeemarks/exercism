defmodule Ledger do
  ...
  defp format_amount(locale, entry_amount_in_cents, currency, number) do
    format_amount2(locale, entry_amount_in_cents, currency, number) |> String.pad_leading(14, " ")
  end

  defp format_amount2(locale, entry_amount_in_cents, currency, number) when entry_amount_in_cents >= 0 do
    if locale == :en_US do
  ...
  defp format_amount2(locale, _entry_amount_in_cents, currency, number) do
    if locale == :en_US do
      " (#{currency_symbol(currency)}#{number})"
    else
      " #{currency_symbol(currency)} -#{number} "
