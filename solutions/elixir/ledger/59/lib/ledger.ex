defmodule Ledger do
  ...
  def format_entries(currency, locale, entries) do
    Ledger.LocaleSettings.locale_settings()[locale][:header] <> Ledger.Entry.format(entries, currency, locale)
  end

  defmodule LocaleSettings do
    def locale_settings do %{ en_US:
                          %{thousands_separator: ",",
  ...
  defmodule EntryDate do
    def format(locale, entry_date) do
      ...
      Ledger.LocaleSettings.locale_settings()[locale].format_date.(day, month, year)
      # format(locale, day, month, year, date_separator)
    end

    defp format(:en_US, day, mon, year, date_separator), do:  "#{mon}#{date_separator}#{day}#{date_separator}#{year} "
    defp format(_, day, mon, year, date_separator), do:       "#{day}#{date_separator}#{mon}#{date_separator}#{year} "
  end
  ...
  defmodule Entry do
    ...
    defp format_entry(entry, currency, locale) do
      ...
      locale_settings = Ledger.LocaleSettings.locale_settings()[locale]
      thousands_separator = locale_settings[:thousands_separator]
      cents_separator = locale_settings[:cents_separator]
