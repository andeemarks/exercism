defmodule PrimeFactors do
  ...
  def factors_for(number) do
    ...
    PrimeGenerator.generate_primes(limit)
    |> find_factors([], number)
    |> Enum.sort()
  end

  defp find_factors([], factors, 1), do: factors
  defp find_factors([], factors, number), do: [number | factors]

  defp find_factors([next_prime | rest], factors, number) when rem(number, next_prime) == 0 do
    find_factors([next_prime | rest], [next_prime | factors], div(number, next_prime))
  end
  defp find_factors([_next_prime | rest], factors, number) do
    find_factors(rest, factors, number)
  end
end
