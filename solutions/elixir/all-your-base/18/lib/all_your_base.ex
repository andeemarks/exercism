defmodule AllYourBase do
  ...
  defp to_base_10(digits, base) do
    ...
    |> Enum.reverse()
    |> Enum.with_index()
    |> Enum.reduce(0, fn {digit, index}, acc -> acc + digit * Integer.pow(base, index) end)
  end

  defp from_base_10(digits, base) when base in 2..36 do
    digits
    |> Integer.to_string(base)
    |> String.graphemes()
    |> Enum.map(fn d -> Integer.parse(d, base) |> elem(0) end)
  end

  defp from_base_10(digits, base) do
    powers = 0..100
    |> Stream.map(&Integer.pow(base, &1))
