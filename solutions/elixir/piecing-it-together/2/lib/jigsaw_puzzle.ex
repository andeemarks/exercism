...
@type t() :: %__MODULE__{
  ...
  def data(spec) when spec.format == :square and not is_nil(spec.columns) and spec.columns != spec.rows do
    { :error, "Contradictory data" }
  end
  def data(spec) when not is_nil(spec.pieces) and not is_nil(spec.aspect_ratio) do
    {rows, columns}  = matching_aspect_ratio_constraint(find_by_target(find_factors(spec.pieces), spec.pieces), spec.aspect_ratio)
  ...
  def data(spec) when not is_nil(spec.border) and spec.format == :portrait and not is_nil(spec.pieces) do
    {side1, side2}  = matching_border_constraint(find_by_target(find_factors(spec.pieces), spec.pieces), spec.border)
  ...
  @spec find_factors(target :: pos_integer()) :: [pos_integer()]
  defp find_factors(target) do
