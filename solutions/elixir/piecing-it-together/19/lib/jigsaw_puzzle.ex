...
@type t() :: %__MODULE__{
  ...
  defp matching_border_constraint(factors, border) do
    factors
    |> Enum.filter(fn {row, column} -> row * 2 + ((column - 2) * 2) == border end)
    |> List.first()
  end

  @spec matching_aspect_ratio_constraint([pos_integer()], pos_integer() | function()) :: {pos_integer(), pos_integer()}
  defp matching_aspect_ratio_constraint(factors, constraint) when is_function(constraint) do
    factors
  ...
  defp matching_aspect_ratio_constraint(factors, aspect_ratio) when aspect_ratio > 1 do
    ...
    matching_aspect_ratio_constraint(factors, fn {row, column} -> max(row, column) / min(row, column) == num / denom end)
  end
  defp matching_aspect_ratio_constraint(factors, aspect_ratio) do
    {num, denom} = Float.ratio(aspect_ratio)
    matching_aspect_ratio_constraint(factors, fn {row, column} -> row / column == num / denom end)
  end
