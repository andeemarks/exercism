...
describe("School", () => {
  ...
  xtest("findStudentGrade", () => {
    expect(school.findStudentGrade("Aimee")).toBeUndefined();
    school.add("Aimee", 2);
    expect(school.findStudentGrade("Aimee")).toEqual("2");
    school.add("Aimee", 1);
    expect(school.findStudentGrade("Aimee")).toEqual("1");
  });

  xtest("removeStudentFromGrade", () => {
    school.add("Aimee", 1);
    expect(school.roster()[1]).toEqual(["Aimee"]);
    school.removeStudentFromGrade("Aimee", "2");
    expect(school.roster()[1]).toEqual(["Aimee"]);
    school.removeStudentFromGrade("Aimee", "1");
    expect(school.roster()[1]).toEqual([]);
  });

  xtest("gradeHasStudent", () => {
    expect(school.gradeHasStudent([], "Aimee")).toBeFalsy();
    expect(school.gradeHasStudent([""], "Aimee")).toBeFalsy();
    expect(school.gradeHasStudent(["Aimee"], "Aimee")).toBeTruthy();
    expect(school.gradeHasStudent(["Aimee", "Beatrice"], "Aimee")).toBeTruthy();
  });
});
